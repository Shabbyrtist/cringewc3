...
local function getFoodBag(p)
    if not playerHandler.IsActive(p) then return nil end
...
function Bag.AddFood(p, foodName)
    ...
    if not foodUnit then return end
    
    table.insert(bag, foodUnit.data)
end
...
-- Извлечь случайного юнита из мешка и вернуть его; если мешок пуст, вернуть nil
